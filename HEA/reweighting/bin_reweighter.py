...
#             assert (plot_reweighted or plot_original)
        ...
        pt.fix_plot(ax, factor_ymax=1., show_leg=False,
                    ymin_to_0=False,
                    pos_text_LHC=None,
                    loc_leg='upper left')
...
#             x+= (high - low) / (4 * self.n_bins) / 2
            ...
            ymin = np.min(spline)*0.7
            ymax = np.max(spline)*1.3
    ...
    ## COMPUTE WEIGHTS ======================================================
    def fit_spline(self, column, k=3, s=None,
                    recompute=False):
    ...
    def apply_new_MC_weights(self, column, fit_spline=False, s=None):
