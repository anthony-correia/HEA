...
def evaluate_pdf_bins(model, edges):
    """ Evaluate a pdf integrating
    over each bin (instead of
    ...
    model: zfit.pdf.BasePDF
        zfit model
    edges: array-like
        Bin edges where to evaluate the pdf
    
    Returns
    -------
    y: array-like
        values of the pdf
        integrated within the range of 
        each bin
    """
    y = []
    ...
    for i in range(len(edges)-1):
        y.append(
            model.analytic_integrate(limits=(edges[i], edges[i+1]))
        )
